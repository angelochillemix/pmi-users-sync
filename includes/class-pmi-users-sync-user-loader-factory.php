	...
	public static function create_user_loader() {
		...
		switch ( $user_loader_option ) {
			...
			case 'option_excel':
				...
				Pmi_Users_Sync_Logger::log_information( __( 'File URL is ', 'pmi-users-sync' ) . $pmi_file_url );
				$file_path = Pmi_Users_Sync_Path_Utils::get_file_path( $pmi_file_url );
				Pmi_Users_Sync_Logger::log_information( __( 'Returned file path is ', 'pmi-users-sync' ) . $file_path );
